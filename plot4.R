...
## The figure is saved as "plot4.png" with the size 480*480 pixels.
##
...
par(mfrow=c(2,2))
 ...
 ## Topleft (almos same as plot2.r, ylabel changed)
...
axis(1, c(1,length(data[["Global_active_power"]])/2,length(data[["Global_active_power"]])), c("Thu","Fri","Sat"))
 ...
 ## Topright (new figure for voltage)
...
axis(1, c(1,length(data[["Voltage"]])/2,length(data[["Voltage"]])), c("Thu","Fri","Sat"))
  ...
  ## Bottomleft (almost same as plot3.r, legend changed)
